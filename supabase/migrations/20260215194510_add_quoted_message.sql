drop function if exists "public"."create_message"(p_team_id uuid, p_content text, p_attachments jsonb, p_parent_id uuid);

alter table "public"."messages" add column "quoted_id" uuid;

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.create_message(p_team_id uuid, p_content text, p_parent_id uuid DEFAULT NULL::uuid, p_quoted_id uuid DEFAULT NULL::uuid, p_attachments jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$declare
...
;
  ...
  create policy "Team members can see typing indicators."
  on "realtime"."messages"
  as permissive
  for select
  to public
using (((extension = ANY (ARRAY['broadcast'::text, 'presence'::text])) AND ("substring"(topic, 'typing-team-(.*)'::text) IN ( SELECT (team_members.team_id)::text AS team_id
